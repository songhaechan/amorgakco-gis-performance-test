...
//    ST_Distance_Sphere() 사용
    @Query(value = "select " +
            ...
            " where ST_Distance_Sphere(kf.point,ST_PointFromText('POINT( 127.0195806)',4326)) < :distance",nativeQuery = true)
    ...
    @Query("select k from KoreanFood k where k.cellId in :tokens")
    List<KoreanFood> findByTokens(@Param("tokens") List<String> tokens);
 }
